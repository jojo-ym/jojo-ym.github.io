var products = [
    {
        "category": "Lifestyle",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike Court Vision low",
                "code": "CVLW",
                "image": "CVLW",
                "price": 3695
            },
            {
                "isAvailable": true,
                "name": "Nike V2K Run",
                "code": "V2KRN",
                "image": "V2KRN",
                "price": 6895
            },
            {
                "isAvailable": true,
                "name": "Nike C1TY",
                "code": "CTY",
                "image": "CTY",
                "price": 5495
            },
            {
                "isAvailable": true,
                "name": "Nike Dunk Low",
                "code": "DNKLW",
                "image": "DNKLW",
                "price": 6895
            },
            {
                "isAvailable": false,
                "name": "Nike Zoom Vomero 5",
                "code": "ZVMR5",
                "image": "ZMVMR",
                "price": 8895
            },
            {
                "isAvailable": true,
                "name": "Nike Air Force 1 '07",
                "code": "AF17",
                "image": "AF17",
                "price": 6195
            },
            {
                "isAvailable": true,
                "name": "Nike T90 SP",
                "code": "T90SP",
                "image": "T90SP",
                "price": 6295
            },
            {
                "isAvailable": true,
                "name": "Nike Shox R4",
                "code": "SHXR4",
                "image": "SHXR4",
                "price": 8395
            },
            {
                "isAvailable": true,
                "name": "Nike Calm",
                "code": "CLM",
                "image": "CLM",
                "price": 4495
            },
            {
                "isAvailable": true,
                "name": "Nike Cortez Textile",
                "code": "CRTZTXTL",
                "image": "cortezTXTL",
                "price": 4995
            },
            {
                "isAvailable": true,
                "name": "Nike Victory One",
                "code": "VCTR1",
                "image": "victory1",
                "price": 1595
            },
            {
                "isAvailable": true,
                "name": "Nike P-6000",
                "code": "P6000",
                "image": "P6000",
                "price": 6195
            },
        ]
    },
    {
        "category": "Jordan",
        "contents": [
            {
                "isAvailable": true,
                "name": "Air Jordan 1 Low",
                "code": "AJ1LW",
                "image": "AJ1LW",
                "price": 5895
            },
            {
                "isAvailable": true,
                "name": "Air Jordan 1 Mid SE",
                "code": "AJ1MDSE",
                "image": "AJ1MDSE",
                "price": 7095
            },
            {
                "isAvailable": true,
                "name": "Air Jordan 1 High OG",
                "code": "AJ1HGHOG",
                "image": "AJ1HGHOG",
                "price": 9895
            },
            {
                "isAvailable": true,
                "name": "Air Jordan 4 RM",
                "code": "AJ4RM",
                "image": "AJ4RM",
                "price": 7895
            },
            {
                "isAvailable": true,
                "name": "Air Jordan 3 Retro 'Rare Air'",
                "code": "AJ3RRA",
                "image": "AJ3RRA",
                "price": 11995
            },
            {
                "isAvailable": true,
                "name": "Jumpman MVP",
                "code": "MVP",
                "image": "MVP",
                "price": 8995
            },
            {
                "isAvailable": true,
                "name": "Jordan CMFT Era",
                "code": "CMFTERA",
                "image": "CMFTERA",
                "price": 6995
            },
            {
                "isAvailable": true,
                "name": "Jordan Spizike Low",
                "code": "SPZKLW",
                "image": "SPZKLW",
                "price": 8895
            },
            {
                "isAvailable": true,
                "name": "Air Jordan 11 Retro 'Legend Blue'",
                "code": "AJ11RLB",
                "image": "AJ11RLB",
                "price": 11895
            },
        ]
    },
    {
        "category": "Running",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike Vaporfly 4",
                "code": "VPFLY4",
                "image": "VPFLY4",
                "price": 13995
            },
            {
                "isAvailable": true,
                "name": "Nike Invicible 3 By You",
                "code": "INVCBL",
                "image": "INVCBL",
                "price": 11995
            },
            {
                "isAvailable": true,
                "name": "Nike Pegasus Trail 5",
                "code": "PGST5",
                "image": "PGST5",
                "price": 7895
            },
            {
                "isAvailable": true,
                "name": "Nike Alphafly 3",
                "code": "ALPHFLY3",
                "image": "ALPHFLY3",
                "price": 14495
            },
            {
                "isAvailable": true,
                "name": "Nike Zoom Fly 6",
                "code": "ZMFLY6",
                "image": "ZMFLY6",
                "price": 9395
            },
            {
                "isAvailable": true,
                "name": "Nike Downshifter 13",
                "code": "DWNSHFTR13",
                "image": "DWNSHFTR13",
                "price": 3295
            },
            {
                "isAvailable": true,
                "name": "Nike Streakfly 2",
                "code": "STRKFLY2",
                "image": "STRKFLY2",
                "price": 10295
            },
            {
                "isAvailable": true,
                "name": "Nike Run Swift 3",
                "code": "RNSWFT3",
                "image": "RNSWFT3",
                "price": 3995
            },
            {
                "isAvailable": true,
                "name": "Nike Juniper Trail 3",
                "code": "JNPRTRL3",
                "image": "JNPRTRL3",
                "price": 4695
            },
        ]
    },
    {
        "category": "Basketball",
        "contents": [
            {
                "isAvailable": true,
                "name": "Tatum 3 PF 'Tunnel Walk'",
                "code": "TT3TW",
                "image": "TT3TW",
                "price": 7095
            },
            {
                "isAvailable": true,
                "name": "Luka .77 PF",
                "code": "LK77PF",
                "image": "LK77PF",
                "price": 5795
            },
            {
                "isAvailable": true,
                "name": "Luka 4 PF 'Space Navigator'",
                "code": "LK4SN",
                "image": "LK4SN",
                "price": 7595
            },
            {
                "isAvailable": true,
                "name": "Luka 3 PF",
                "code": "LK3PF",
                "image": "LK3PF",
                "price": 7395
            },
            {
                "isAvailable": true,
                "name": "Tatum 2 'Sidewalk Chalk' PF",
                "code": "TTM2SC",
                "image": "TTM2SC",
                "price": 7095
            },
            {
                "isAvailable": true,
                "name": "Zion 3 'Rising' PF",
                "code": "ZN3R",
                "image": "ZN3R",
                "price": 8395
            },
            {
                "isAvailable": true,
                "name": "Zion 4 EP 'Sharonda'",
                "code": "ZN4SHR",
                "image": "ZN4SHR",
                "price": 8095
            },
            {
                "isAvailable": true,
                "name": "Jordan 'Why not?' Zer0.3 PF",
                "code": "JRDNWN",
                "image": "JRDNWN",
                "price": 7395
            },
            {
                "isAvailable": true,
                "name": "Jordan Zoom Separate PF",
                "code": "JRDNZS",
                "image": "JRDNZS",
                "price": 5895
            },
            {
                "isAvailable": true,
                "name": "Jordan Heir Series PF Bloodline'",
                "code": "JRDNHS",
                "image": "JRDNHS",
                "price": 6195
            },
            {
                "isAvailable": true,
                "name": "Tatum 1 'Denim' PF",
                "code": "TTM1DNM",
                "image": "TTM1DNM",
                "price": 6895
            },
            {
                "isAvailable": true,
                "name": "Air Jordan XXXIX PF 'Noir'",
                "code": "XXXIXPFN",
                "image": "XXXIXPFN",
                "price":11395
            },
        ]
    },
    {
        "category": "Football",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike Mercurial Vapor 16 Elite",
                "code": "MV16EKM",
                "image": "MV16EKM",
                "price": 13595
            },
            {
                "isAvailable": true,
                "name": "Nike Phantom GX 2 Elite",
                "code": "PNTMGX2E",
                "image": "PNTMGX2E",
                "price": 13395
            },
            {
                "isAvailable": true,
                "name": "Nike Mercurial Superfly 10 Academy",
                "code": "MCSPFL10",
                "image": "MCSPFL10",
                "price": 5495
            },
            {
                "isAvailable": true,
                "name": "Nike Tiempo Legend 10 Elite",
                "code": "TL10E",
                "image": "TL10E",
                "price": 12295
            },
            {
                "isAvailable": true,
                "name": "Nike Mercurial Superfly 10 Elite",
                "code": "MCS10E",
                "image": "MCS10E",
                "price": 14895
            },
            {
                "isAvailable": true,
                "name": "Nike Mercurial Vapor 1 RGN SE",
                "code": "MCV1RSE",
                "image": "MCV1RSE",
                "price": 16895
            },
        ]
    },
    {
        "category": "Training",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike Air Monarch IV",
                "code": "AMIV",
                "image": "AMIV",
                "price": 2595
            },
            {
                "isAvailable": true,
                "name": "Nike MC Trainer 3",
                "code": "MCT3",
                "image": "MCT3",
                "price": 3695
            },
            {
                "isAvailable": true,
                "name": "Nike Air Max Alpha Trainer 6",
                "code": "AMAT6",
                "image": "AMAT6",
                "price": 4995
            },
            {
                "isAvailable": true,
                "name": "Nike Free Metcon 6",
                "code": "FRMTCN6",
                "image": "FRMTCN6",
                "price": 6895
            },
            {
                "isAvailable": true,
                "name": "Nike Free 2025",
                "code": "FR2025",
                "image": "FR2025",
                "price": 5795
            },
            {
                "isAvailable": true,
                "name": "Nike Motiva",
                "code": "MTV",
                "image": "MTV",
                "price": 6195
            },
        ]
    },
    {
        "category": "Skateboarding",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike SB Dunk Low Pro",
                "code": "SBDNKLWP",
                "image": "SBDNKLWP",
                "price": 6195
            },
            {
                "isAvailable": true,
                "name": "Nike SB Force 58",
                "code": "SBF58",
                "image": "SBF58",
                "price": 3895
            },
            {
                "isAvailable": true,
                "name": "Nike SB FC Classic",
                "code": "SBFCC",
                "image": "SBFCC",
                "price": 3695
            },
            {
                "isAvailable": true,
                "name": "Nike SB Zoom Blazer Low Pro GT",
                "code": "SBZMBLWPG",
                "image": "SBZMBLWPG",
                "price": 3995
            },
            {
                "isAvailable": true,
                "name": "Nike SB Alleyoop",
                "code": "SBALYP",
                "image": "SBALYP",
                "price": 4095
            },
            {
                "isAvailable": true,
                "name": "Nike SB Zoom Janoski OG+",
                "code": "SBZMJOG",
                "image": "SBZMJOG",
                "price": 5195
            },
        ]
    },
    {
        "category": "Golf",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike Victory Tour 4",
                "code": "VCTRYT4",
                "image": "VCTRYT4",
                "price": 10895
            },
            {
                "isAvailable": true,
                "name": "Nike Air MAx 90 G",
                "code": "AM90G",
                "image": "AM90G",
                "price": 8595
            },
            {
                "isAvailable": true,
                "name": "Nike Free Golf NN",
                "code": "FGNN",
                "image": "FGNN",
                "price": 7395
            },
            {
                "isAvailable": true,
                "name": "Jordan NU Retro 1 G",
                "code": "NURT1G",
                "image": "NURT1G",
                "price": 8595
            },
            {
                "isAvailable": true,
                "name": "Nike Infinity Tour 2",
                "code": "INFNTYT2",
                "image": "INFNTYT2",
                "price": 9395
            },
            {
                "isAvailable": true,
                "name": "Nike Victory Tour 3 Boa",
                "code": "VCTRYT3B",
                "image": "VCTRYT3B",
                "price": 11295
            },
        ]
    },
    {
        "category": "Tennis",
        "contents": [
            {
                "isAvailable": true,
                "name": "NikeCourt Lite 4",
                "code": "CRTLT4",
                "image": "CRTLT4",
                "price": 4095
            },
            {
                "isAvailable": true,
                "name": "Nike GP Challenge Pro",
                "code": "GPCPRO",
                "image": "GPCPRO",
                "price": 6195
            },
            {
                "isAvailable": true,
                "name": "Nike Zoom GP Challenge 1",
                "code": "ZMGPC1",
                "image": "ZMGPC1",
                "price": 8395
            },
            {
                "isAvailable": true,
                "name": "Nike Vapor Pro 3",
                "code": "VPRPRO3",
                "image": "VPRPRO3",
                "price": 6995
            },
            {
                "isAvailable": true,
                "name": "Nike Vapor 12 Premium",
                "code": "VPR12P",
                "image": "VPR12P",
                "price": 9695
            },
            {
                "isAvailable": true,
                "name": "Nike GP Challenge Pro Premium",
                "code": "GPCPROP",
                "image": "GPCPROP",
                "price": 6995
            },
        ]
    },
    {
        "category": "Athletics",
        "contents": [
            {
                "isAvailable": true,
                "name": "Nike Vaporfly 4 'J Ingebrigtsen'",
                "code": "VPRFLY4JI",
                "image": "VPRFLY4JI",
                "price": 13995
            },
            {
                "isAvailable": true,
                "name": "Nike Streakfly 2",
                "code": "STREAKFLY2",
                "image": "STREAKFLY2",
                "price": 10295
            },
            {
                "isAvailable": true,
                "name": "Nike Rival Fly 4",
                "code": "RVLFLY4",
                "image": "RVLFLY4",
                "price": 5495
            },
            {
                "isAvailable": true,
                "name": "Nike Alphafly 3 Premiuim",
                "code": "ALPH3PRE",
                "image": "ALPH3PRE",
                "price": 15095
            },
            {
                "isAvailable": true,
                "name": "Nike Zoom Fly 6 'J Ingebrigtsen'",
                "code": "VPR12P",
                "image": "VPR12P",
                "price": 9895
            },
            {
                "isAvailable": true,
                "name": "Nike Vaporfly 3 'E Kipchoge'",
                "code": "GPCPROP",
                "image": "GPCPROP",
                "price": 12595
            },
        ]
    },
];

var total = 0;

function loadCategories() {
    var categoriesContainer = document.getElementById("categories");

    products.forEach((product, index) => {
        categoriesContainer.innerHTML += `
          <div onclick="loadProducts('`+ index + `')" class="card shadow-sm mx-1 custom-button-categ text-center d-flex justify-content-center" style="cursor: pointer; padding: 2px; height: 50px; width: 150px; background-color: rgb(249, 246, 246); color: rgb(8,8,8)">
            <small class="categoryName" style="font-weight: 400;">`+ product.category + `</small>
          </div>
        `;
    });
}



function loadProducts(categoryIndex) {
    const productContainer = document.getElementById("productContainer");
    productContainer.innerHTML = "";
    products[categoryIndex].contents.forEach((content) => {
        productContainer.innerHTML +=
            `  <div onclick="addToReceipt('` + content.price + `','` + content.code + `')"
                                        class="card shadow mx-1 my-2 custom-button content p-3"
                                        style="height: 45vh; background-color: white;">
                                        <img src="image/`+ content.image + `.avif" alt="" width="100%">
                                        <small class="contentName" style="font-weight: 500; padding-top: 7px; color: rgb(8,8,8)">`+ content.name + `</small>
                                        <small class="contentPrice" style="color: rgb(8,8,8)">PHP `+ content.price + `</small>
                </div>
     `;
    });
}


function addToReceipt(price, code) {
    var receiptContainer = document.getElementById("receipt");
    total = parseFloat(total) + parseFloat(price);

    totalValueElement = document.getElementById("totalValue");
    totalValueElement.innerHTML = `PHP `+total;

    receiptContainer.innerHTML += `
            <div class="d-flex flex-row justify-content-between px-4">
                <div class="small receiptCode" style="color: rgb(8,8,8)">`+ code + `</div>
                <div class="small receiptPrice" style="color: rgb(8,8,8)">PHP `+ price + `</div>  
            </div>      
      `;
}

loadCategories();